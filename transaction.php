    ...
    <div class="content">
        ...
        <div class="block table-responsive">
            <div class="block-header block-header-default">
                <h3 class="block-title">Transaction Data</h3>
                <div class="block-options">
            ...
            <div class="block-content">
                  ...
                  //for total count data
                    if(@$_REQUEST['s']=='1091vdf8ame151') {
                        ...
                        $sql = "SELECT trx_id, trx_type, partner_name, customer_phone_number, trx_amount, DATE_FORMAT(DATE_ADD(created_date,INTERVAL '7' HOUR), '%d/%m/%Y on %H:%i') as created_date, merchant_name, payment_channel, trx_purchase_item, sn_client, sn_trx FROM tbl_trx INNER JOIN tbl_merchant_code USING (merchant_code)  WHERE (trx_type LIKE '%$txt_search%' OR customer_phone_number LIKE '%$txt_search%' OR sn_trx LIKE '%$txt_search%' OR sn_client LIKE '%$txt_search%' OR merchant_name LIKE '%$txt_search%' OR payment_channel LIKE '%$txt_search%') LIMIT $offset, $dataPerPage";
                    }elseif (@$_REQUEST['s']=='1'){
                        $txt_search         = input_data(filter_var($_REQUEST['txt_search'],FILTER_SANITIZE_STRING));
                        ...
                        $sql  = "SELECT trx_id, trx_type, partner_name, customer_phone_number, trx_amount, DATE_FORMAT(DATE_ADD(created_date,INTERVAL '7' HOUR), '%d/%m/%Y on %H:%i') as created_date, merchant_name, payment_channel, trx_purchase_item, sn_client, sn_trx FROM tbl_trx INNER JOIN tbl_merchant_code USING (merchant_code)  WHERE date(created_date) = '".$created_date_f."' LIMIT $offset, $dataPerPage";
                    }else{
                        $sql = "SELECT trx_id, trx_type, partner_name, customer_phone_number, trx_amount, DATE_FORMAT(DATE_ADD(created_date,INTERVAL '7' HOUR), '%d/%m/%Y on %H:%i') as created_date, merchant_name, payment_channel, trx_purchase_item, sn_client, sn_trx FROM tbl_trx INNER JOIN tbl_partner using (partner_key) INNER JOIN tbl_merchant_code USING (merchant_code)  LIMIT $offset, $dataPerPage";
                    }
        ...
        if(@$_REQUEST['s']=='1091vdf8ame151') {
            $txt_search            = input_data(filter_var($_REQUEST['txt_search'],FILTER_SANITIZE_STRING));                   
          $sql = "SELECT count(trx_id) as jumData FROM tbl_trx INNER JOIN tbl_merchant_code USING (merchant_code)  WHERE  (trx_type LIKE '%$txt_search%' OR customer_phone_number LIKE '%$txt_search%' OR sn_trx LIKE '%$txt_search%' OR sn_client LIKE '%$txt_search%' OR merchant_name LIKE '%$txt_search%' OR payment_channel LIKE '%$txt_search%')";
        }elseif(@$_REQUEST['s']=='1') {
            $txt_search            = input_data(filter_var($_REQUEST['txt_search'],FILTER_SANITIZE_STRING));
          ...
          $sql = "SELECT count(trx_id) as jumData FROM tbl_trx WHERE  date(created_date) = '".$created_date_f."'";                    
        }else{            
          $sql = "SELECT count(trx_id) as jumData FROM tbl_trx FROM tbl_trx";
        }          
